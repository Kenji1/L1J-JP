...
public class L1MaterialChoice {
	...
	private static final String PATH = "./data/xml/Item/MaterialChoice.xml";

	private static final HashMap<Integer, L1MaterialChoice> _dataMap = new HashMap<Integer, L1MaterialChoice>();
	...
	private boolean init() {
		...
		for (Effect each : getEffects()) {
			...
			if (ItemTable.getInstance().getTemplate(each.getNeedItemId()) == null) {
				System.out.println("アイテムID " + each.getNeedItemId() + " のテンプレートが見つかりません。");
				return false;
			}
		}
		return true;
	...
	public static void load() {
		...
		System.out.print("loading material choice...");
		try {
